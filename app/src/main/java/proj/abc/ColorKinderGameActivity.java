 ...
 */
public class ColorKinderGameActivity   extends Activity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    setContentView(R.layout.activity_color_kinder);
    ...
    HashMap<Integer,String> colorData = GameMap.getInstance().getColorOutlineData();
    ...
    for(Map.Entry<Integer, String> entry : colorData.entrySet()){
      ...
      if(count == questions[currQuestion]){
        ...
        imgButton.setImageResource( getColor(currColor));
        imgColor.setImageResource((Integer) entry.getKey());
    ...
    String[] colorList = new String[]{"red","blue","yellow","orange","purple","green"};

    String randomColor ;

    do{
      randomColor = colorList[((int) (Math.random()*colorList.length))];
    }while(randomColor.equals(currColor));
    
    imgBtns.get(0).setImageResource(getColor(randomColor));
    imgBtns.get(0).setOnClickListener(new View.OnClickListener() {
      @Override
      public void onClick(View v) {
        playWrong();
      }
